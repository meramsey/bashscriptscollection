...
#Snapshot subfolder customization. Our backups for this server are all inside the subfolder coby-backups on the backup server. If no subfolder is needed this can be commented out or left blank.
SnapShotPathSubFolder=coby-backups
...
unionfs-fuse -o cow,max_files=32768 \
             -o allow_other,use_ino,suid,dev,nonempty \
             $MountPath1=RW:$MountPath2=RW:"${dirs[0]}"$SnapShotPathSubFolder=RO:"${dirs[1]}"$SnapShotPathSubFolder=RO:"${dirs[2]}"$SnapShotPathSubFolder=RO:"${dirs[3]}"$SnapShotPathSubFolder=RO:"${dirs[4]}"$SnapShotPathSubFolder=RO:"${dirs[5]}"$SnapShotPathSubFolder=RO:"${dirs[6]}"$SnapShotPathSubFolder=RO \
             $UnionMountMergePath
