...
## https://gitlab.com/cyberpaneltoolsnscripts/cyberpanel_backup_all_dbs
## Objective Enable backing up all cyberpanel users databases via cyberpanel user/login or DBUSER with priveleges to all databases. This script utilizes uapi to list the current DB.
## How to use. 
# ./cyberpanel_backup_all_dbs.sh
# bash cyberpanel_backup_all_dbs.sh
# Example cron to use if file is located in main home dir of user. Make sure to "chmod +x cyberpanel_backup_all_dbs.sh" the file before setting as a cron.
# 0 2 * * * /bin/bash ${HOME}/cyberpanel_backup_all_dbs.sh



#######Start Configuring###############
#Set the cyberpanel login here or a db user with priveleges to all databases for backup purposes
DBUSER=""
PASSWORD=""
...
if [ "$DBUSER" = "" -o "$DBUSER" == 'username' ] && [ ! -f "$MySQL_CNF" ]; then
    echo -n "`tput setaf 2`What database user or cyberpanel ${USER} user would you like to use for backing up databases?: `tput setaf 7`"
	read DBUSER
fi
...
for DB in $(cat ~/databases_to_backup.txt| tr '/' '\n'); do 
